...
def main(learning_rate, penalty, sensors, data, filters=64, train=False):
    ...
    def build_cnn(input_var=None):
        ...
        net['input'] = lasagne.layers.InputLayer(
            ...
            input_var=input_var)
        net['conv1/5x1'] = lasagne.layers.Conv2DLayer(net['input'], NUM_FILTERS,
                                                      (FILTER_SIZE, 1))
        ...
        net['conv4/5x1'] = lasagne.layers.Conv2DLayer(net['conv3/5x1'],
                                                      NUM_FILTERS,
                                                      (FILTER_SIZE, 1))
        net['shuff'] = lasagne.layers.DimshuffleLayer(net['conv4/5x1'],
                                                      (0, 2, 1, 3))
        net['lstm1'] = lasagne.layers.LSTMLayer(
            lasagne.layers.dropout(net['shuff'], p=.5), NUM_UNITS_LSTM)
        net['lstm2'] = lasagne.layers.LSTMLayer(
            lasagne.layers.dropout(net['lstm1'], p=.5), NUM_UNITS_LSTM)
        ...
        net['shp1'] = lasagne.layers.ReshapeLayer(net['lstm2'],
                                                  (-1, NUM_UNITS_LSTM))
        net['prob'] = lasagne.layers.DenseLayer(
            lasagne.layers.dropout(net['shp1'], p=.5), NUM_CLASSES,
            nonlinearity=lasagne.nonlinearities.softmax)
        # Tensors reshaped back to the original shape
        net['shp2'] = lasagne.layers.ReshapeLayer(net['prob'], (
            BATCH_SIZE, FINAL_SEQUENCE_LENGTH, NUM_CLASSES))
    ...
    for i in range(len(batches_test['X'])):
        ...
        test_acc += acc
        test_batches += 1
        test_conf_matrix += ConfusionMatrix(batches_test['y'][i],
                                            pred_labels_fn(
                                                batches_test['X'][i])[0])

    # save test conf matrix
    save_as = 'test_acc' + str(test_acc / test_batches * 100)
    save_as += 'epoch' + str(epoch)
    ...
    # save for plots
    save_info((validation_loss, validation_acc),
              (str(penalty), str(learning_rate)))
    ...
    name = 'test_acc' + str(test_acc / test_batches * 100)
    name += 'penalty' + str(penalty) + 'learning_rate' + str(
        learning_rate) + 'model.npz'
...
if __name__ == '__main__':
    random.seed(337)
    # LOAD FULL DATASET
    X_train = np.loadtxt('./Project Data/X_train.csv').astype(np.float32)
    y_train = np.loadtxt('./Project Data/Y_train.csv').astype(np.uint8)

    X_val = np.loadtxt('./Project Data/X_val.csv').astype(np.float32)
    y_val = np.loadtxt('./Project Data/Y_val.csv').astype(np.uint8)

    X_test = np.loadtxt('./Project Data/X_test.csv').astype(np.float32)
    y_test = np.loadtxt('./Project Data/Y_test.csv').astype(np.uint8)

    # train with suggested parameters
    learning = 0.0001
    penalty = 0.0001
    main(learning, penalty, 16, (X_train, y_train, X_val, y_val, X_test,
                                     y_test))

    # parameter search 10 pairs of numbers
    for learning, penalty in zip((random.uniform(0.01, 0.0001) for x in
                                 xrange(10)),
                               (random.uniform(0.001, 0.00001) for x in
                                xrange(10))):
    ...
    learning = 0.0001
    penalty = 0.0001
    main(learning, penalty, 12, (X_train, y_train, X_val, y_val, X_test,
                             y_test))
